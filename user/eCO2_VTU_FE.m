function gp = eCO2_VTU_FE(gp) %configuration
...
gp.runcontrol.pop_size = 250;	%250	种群规模，即每代中个体（数学模型）的数量。较大的种群能增加多样性，但计算成本更高。	                  				   
gp.runcontrol.timeout = 40;   % 单次GP运行的超时时间s。算法会在达到时间后终止。
gp.runcontrol.runs = 3; %3 独立运行的次数。多次运行可减少随机性影响，提高找到优质解的概率
% 总生成数量 = pop_size * runs

%selection
gp.selection.tournament.size = 25; %25锦标赛选择的竞争池大小。每次从种群中随机选取25个个体，从中选出最优者进入下一代。 
...
gp.nodes.const.p_int= 0.4;  %生成常数节点时，60%概率为整数，40%为浮点数。影响模型中的常数类型，适用于特定问题（如离散数据）。
%调整意义：控制模型常数的多样性。

%fitness 
gp.fitness.terminate = true; %启用适应度终止条件。当适应度达到阈值时，提前终止运行。
gp.fitness.terminate_value = 0.05; % 0.2适应度阈值（如均方误差MSE）。若某次运行的适应度≤0.05，则停止。你的调整：从0.2降低到0.05，要求模型更精确。
...
gp.userdata.xtrain = train_VTU_FE(:,1:3); 
gp.userdata.ytrain = train_VTU_FE(:,4); 
gp.userdata.xtest = test_VTU_FE(:,1:3); 
gp.userdata.ytest = test_VTU_FE(:,4); 
gp.userdata.name = 'eCO2R_VTU_FE';
